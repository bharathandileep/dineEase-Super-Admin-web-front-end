...
export function WizardForm({ initialData }: WizardFormProps) {
  ...
  const validateStep1 = () => {
    ...
    if (!formData.organizationName) {
      ...
      newErrors.organizationName = "Only alphabets are allowed";
    }

    if (!formData.managerName) {
      newErrors.managerName = "Required";
    } else if (!/^[A-Za-z\s]+$/.test(formData.managerName)) {
      newErrors.managerName = "Only alphabets are allowed";
    }
  ...
  const validateStep2 = () => {
    ...
    if (!formData.panCardUserName) {
      newErrors.panCardUserName = "Required";
    } else if (!/^[A-Za-z\s]+$/.test(formData.panCardUserName)) {
      newErrors.panCardUserName = "Only alphabets are allowed";
    }
