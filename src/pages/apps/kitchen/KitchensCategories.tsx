  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      ...
      try {
        ...
        const response = await kitchensGetAllCategories(query);
        if (response.status) {
          setMenuItems(response.data.categories);
          setTotalPages(response.data.pagination.totalPages);
          setTotalItems(response.data.pagination.totalItems);
        } else {
          toast.error("Failed to load kitchen categories.");
        }
      } catch (error: any) {
  ...
  return (
    <>
      <div className="container py-2">
        <PageTitle
          breadCrumbItems={[
            { label: "Kitchens", path: "/apps/kitchens/category" },
            { label: "Category", path: "/apps/kitchens/category", active: true },
          ]}
          ...
          <div className="table-responsive">
            {loading ? (
              <div className="text-center my-4">
                ...
                <p>Loading kitchen categories...</p>
              </div>
            ) : menuItems.length === 0 ? (
              <div className="text-center my-4">
                <p>No Kitchen Categories Found</p>
              </div>
            ) : (
              <Table
                columns={columns}
